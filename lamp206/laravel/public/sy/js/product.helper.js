...
var isCountDown = (pageUrl.indexOf("countdownproductsdetails") > -1 || pageUrl.indexOf("countdownproduct_detail") > -1);
var isGroupBuy = (pageUrl.indexOf("groupbuyproductdetails") > -1 || pageUrl.indexOf("groupbuyproduct_detail") > -1);
...
$(document).ready(function () {
    ...
    $("#imgCloseLogin").bind("click", function () { $("#loginForBuy").hide(); $(".modal_qt").remove(); });
    $("#btnLoginAndBuy").bind("click", function () { LoginAndBuy(); });
    $("#btnAmoBuy").bind("click", function () { AnonymousBuy(); });//立即购买弹框中的匿名购买
    $("#liAmoBuy").bind("click", function () { AnonymousBuy(); });//匿名购买
...
function ResetSkuRowClass(skuRowId, skuSelectId) {
    ...
    $.ajax({
        url: "ShoppingHandler.aspx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "UnUpsellingSku", productId: $("#hiddenpid").val(), AttributeId: pvid[1], ValueId: pvid[2], sourceId: sourceId },
        success: function (resultData) {
...
function ResetCurrentSku(skuId, sku, weight, stock, alertstock, salePrice, options) {
    ...
    if ($("#referSpan").length > 0) {
        GetReferDedut($("#hiddenpid").val(), skuId);
